...
3. **Install chart-testing** (gated on `mode == 'changed-charts'`) — detects host arch, downloads `chart-testing_<X.Y.Z>_linux_<arch>.tar.gz` from `github.com/helm/chart-testing/releases/download/<ct_version>/...`, sha256-verifies it against the release `checksums.txt` (exact-filename `awk` match, fails with `::error::` on mismatch) **before** extracting and `sudo install -m 0755 ct /usr/local/bin/ct`. ct loads `chart_schema.yaml` + `lintconf.yaml` from `/etc/ct/` at runtime — these ship in the tarball's `etc/` dir (NOT embedded in the binary), so the step `sudo install`s both into `/etc/ct/`, guarding first that the tarball actually contained them (a missing `etc/` schema fails with a clear `::error::` instead of an opaque `install: cannot stat`). Also `pip install`s `yamale` + `yamllint`, which ct shells out to.
4. **List changed charts** (`id: discover`, gated on `mode == 'changed-charts'`) — `ct list-changed --target-branch "$BASE_REF" --chart-dirs "$CHARTS_DIR"` with its stderr + exit code captured (no blanket `|| true`): a non-zero exit **combined with** a `fatal|error|unknown revision|does not exist` stderr keyword fails the step with a clear `::error::` (e.g. the caller forgot `fetch-depth: 0`, or `base_ref` is unreachable), while a clean "no changed charts" run (empty output, no error keyword) still falls through to `changed=false`/`count=0`. Writes `changed=true|false`, `count=<N|0>`, `charts=<multiline>` to `$GITHUB_OUTPUT`. Downstream `lint` / `template` / `kubeconform` steps read `steps.discover.outputs.{changed,charts}` via the `env:` block. Also drives the `changed_charts_count` action output.
...
- `kubeconform_result` is set by the `kubeconform` step, which always runs. Same three-value shape (`success` / `failure` / `skipped`). The install sub-stage (`set -e -o pipefail` around curl+install) is intentionally inside the same step: an unsupported arch or a failed download/install now writes `kubeconform_result=failure` to `$GITHUB_OUTPUT` **before** exiting non-zero, so the single output stays populated on the install-failure path too. As a defensive fallback, callers should still treat an empty `kubeconform_result` the same as `failure` for the rare case where `set -e` aborts before the result line is reached (e.g. `mktemp` / `kubeconform -v`).
...
All three result-bearing steps (`lint`, `template`, `kubeconform`) share the same "write output before exit" pattern: `set +e` then `set -uo pipefail` around the actual work (the `-e` stays off so the result line is always reached; `-u` catches env-var typos; `pipefail` makes a masked pipe failure such as `helm template … | kubeconform …` surface in `$?`), capture `$?`, write the branch's result to `$GITHUB_OUTPUT`, then `exit $RC`. This keeps the single-step-id rule intact on both success and failure paths.
